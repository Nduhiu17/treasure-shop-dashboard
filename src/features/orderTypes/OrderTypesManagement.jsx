  ...
  return (
    <Card className="m-1 xs:m-2 sm:m-4 p-1 xs:p-2 sm:p-6 shadow-lg border-0 w-full max-w-none">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-900">Order Types Management</h2>
        <Button
      ...
      {loading ? (
        <div className="text-center py-8 text-gray-500">Loading order types...</div>
      ) : error ? (
        <div className="text-center py-8 text-red-600">{error}</div>
      ) : (
        <div className="rounded-2xl border border-gray-200 bg-white/90 shadow-lg w-full min-h-[320px]" style={{ height: '60vh' }}>
            ...
            <table className="w-full border-separate border-spacing-y-0 rounded-2xl overflow-hidden shadow-xl bg-white">
              <thead className="sticky top-0 bg-gradient-to-r from-gray-100 to-gray-50 z-10">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-bold text-gray-800 uppercase tracking-wider border-b border-gray-200">Base Price/Page</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {orderTypes.length > 0 ? (
                  orderTypes.map((type, idx) => (
                    <tr key={type.id} className={`transition group ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100`} style={{ boxShadow: '0 1px 4px 0 rgba(30, 41, 59, 0.04)' }}>
                      <td className="px-6 py-4 font-semibold text-gray-900 capitalize align-middle">{type.name}</td>
