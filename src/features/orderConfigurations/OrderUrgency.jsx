...
const OrderUrgency = () => {
  ...
  return (
    <React.Fragment>
      ...
      {loading ? (
        <div className="text-center py-8 text-blue-700">Loading order urgency options...</div>
      ) : error ? (
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full border-separate border-spacing-y-0 rounded-2xl overflow-hidden shadow-xl bg-white">
            ...
            <tbody className="divide-y divide-blue-100">
              {orderUrgency.length > 0 ? orderUrgency.map((urg, idx) => (
                <tr key={urg.id} className={`transition group ${idx % 2 === 0 ? 'bg-white' : 'bg-blue-50'} hover:bg-blue-100`} style={{ boxShadow: '0 1px 4px 0 rgba(30, 64, 175, 0.04)' }}>
                  <td className="px-6 py-4 font-semibold text-blue-900 capitalize align-middle">{urg.name}</td>
                  ...
                  <td className="px-6 py-4 text-right align-middle">
                    <div className="flex items-center gap-2 justify-end">
                      <button
                        ...
                        title="Edit order urgency"
                        onClick={e => {
                          e.preventDefault();
                          setEditTarget(urg);
                          setEditForm({ name: urg.name, description: urg.description || '', multiplier: urg.urgency_price_multiplier });
                          setEditOpen(true);
                        }}
                      >
      ...
      {/* Edit Order Urgency Dialog */}
      {editOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-auto">
            <h3 className="text-lg font-bold mb-4 text-blue-900">Edit Order Urgency</h3>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                ...
                try {
                  const jwt = localStorage.getItem("jwt_token");
                  const res = await fetch(`${API_BASE_URL}/api/admin/order-urgency/${editTarget.id}`, {
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: jwt ? `Bearer ${jwt}` : "",
                    },
                    body: JSON.stringify({ name: editForm.name, description: editForm.description, urgency_price_multiplier: parseFloat(editForm.multiplier) }),
                  });
                  if (!res.ok) throw new Error("Failed to update order urgency");
                  showToast({ message: "Order urgency updated successfully", type: "success" });
                  setEditOpen(false);
                  setEditTarget(null);
                  fetchOrderUrgency();
                } catch (err) {
                  showToast({ message: err.message || "Failed to update order urgency", type: "error" });
                } finally {
                  setEditing(false);
                }
              }}
            ...
            >
              ...
              <div className="flex flex-row justify-end gap-2 mt-2">
                ...
                <tr>
                  <td colSpan={3} className="text-center text-xs xs:text-sm sm:text-base px-4 py-1">No order urgency options found.</td>
                </tr>
              )}
            </tbody>
          </table>
