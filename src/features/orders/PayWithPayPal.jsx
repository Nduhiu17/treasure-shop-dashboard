...
export default function PayWithPayPal({ orderId, amount, onSuccess, onCancel }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      {paying && <div className="mt-2 text-blue-600 flex items-center gap-2">Processing payment...</div>}
      {error && <div className="mt-2 text-red-600 text-sm">{error}</div>}
      <button className="text-blue-500 underline mt-4" onClick={onCancel}>Cancel and return</button>
    </div>
  );
}
