...
const OrderStyles = () => {
  ...
  const fetchOrderStyles = () => {
    setLoading(true);
    setError("");
    fetch("http://localhost:8080/api/order-styles")
      .then((res) => res.json())
  ...
  const handleCreateOrderStyle = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8080/api/admin/order-styles", {
        method: "POST",
        headers: {
        ...
        <div className="rounded-2xl border border-blue-100 bg-white/90 shadow-lg w-full min-h-[320px]" style={{ height: '60vh' }}>
          <div className="overflow-x-auto h-full">
            ...
            <div className="overflow-y-auto" style={{ maxHeight: 'calc(60vh - 56px)' }}>
              <table className="w-full min-w-[800px] text-xs xs:text-sm sm:text-base table-fixed">
                <tbody>
                  {orderStyles.length > 0 ? orderStyles.map((style) => (
                    <tr key={style.id} className="hover:bg-blue-50 h-10">
                      <td className="max-w-[120px] truncate text-xs xs:text-sm sm:text-base px-4 py-1 align-middle w-1/4">{style.name}</td>
                      <td className="text-xs xs:text-sm sm:text-base px-4 py-1 align-middle w-2/4">{style.description}</td>
