  ...
  return (
    <React.Fragment>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-900">Order Languages Management</h2>
        <Button
      ...
      {loading ? (
        <div className="text-center py-8 text-gray-500">Loading order languages...</div>
      ) : error ? (
          ...
          <table className="w-full border-separate border-spacing-y-0 rounded-2xl overflow-hidden shadow-xl bg-white">
            ...
            <tbody className="divide-y divide-gray-100">
              {orderLanguages.length > 0 ? orderLanguages.map((lang, idx) => (
                <tr key={lang.id} className={`transition group ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100`} style={{ boxShadow: '0 1px 4px 0 rgba(30, 41, 59, 0.04)' }}>
                  <td className="px-6 py-4 font-semibold text-gray-900 capitalize align-middle">{lang.name}</td>
