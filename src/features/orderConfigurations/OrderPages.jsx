...
const OrderPages = () => {
  ...
  const [editForm, setEditForm] = useState({ name: '', description: '', numberOfPages: '' });
  ...
  const handleCreateOrderPage = async (e) => {
    e.preventDefault();
    setCreating(true);
    const name = nameRef.current.value.trim();
    const description = descRef.current.value.trim();
    const numberOfPages = parseInt(numPagesRef.current.value, 10);
    if (!name) {
    ...
    if (isNaN(numberOfPages) || numberOfPages <= 0) {
      showToast({ message: "Number of pages is required and must be a positive integer", type: "error" });
      setCreating(false);
      return;
    }
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/admin/order-pages`, {
        ...
        body: JSON.stringify({ name, description, number_of_pages: numberOfPages }),
      });
      if (!res.ok) throw new Error("Failed to create order page");
      showToast({ message: "Order page created successfully", type: "success" });
      setDialogOpen(false);
      fetchOrderPages();
    } catch (err) {
      showToast({ message: err.message || "Failed to create order page", type: "error" });
    } finally {
      setCreating(false);
    }
  };
  ...
  // Delete Order Page
  const handleDeleteOrderPage = async () => {
    if (!deleteTarget) return;
    setDeleting(true);
    try {
      const jwt = localStorage.getItem("jwt_token");
      const res = await fetch(`${API_BASE_URL}/api/admin/order-pages/${deleteTarget.id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: jwt ? `Bearer ${jwt}` : "",
      ...
      showToast({ message: `Order page '${deleteTarget.name}' deleted successfully`, type: "success" });
      setOrderPages(orderPages => orderPages.filter(op => op.id !== deleteTarget.id));
  ...
  return (
    <>
      <ConfirmDialog
        ...
        message={deleteTarget ? `Are you sure you want to delete order page '${deleteTarget.name}'? This action cannot be undone.` : ''}
        confirmText={deleting ? "Deleting..." : "Delete"}
        cancelText="Cancel"
        loading={deleting}
        onCancel={() => { setConfirmOpen(false); setDeleteTarget(null); }}
        onConfirm={handleDeleteOrderPage}
      />
      <Card className="m-1 xs:m-2 sm:m-4 p-1 xs:p-2 sm:p-6 shadow-lg border-0 w-full max-w-none">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-900">Order Pages Management</h2>
        <Button
      ...
      {loading ? (
        <div className="text-center py-8 text-gray-500">Loading order pages...</div>
      ) : error ? (
          ...
          <table className="w-full border-separate border-spacing-y-0 rounded-2xl overflow-hidden shadow-xl bg-white">
            <thead className="sticky top-0 bg-gradient-to-r from-gray-100 to-gray-50 z-10">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-xs font-bold text-gray-800 uppercase tracking-wider border-b border-gray-200"># Pages</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {orderPages.length > 0 ? orderPages.map((page, idx) => (
                <tr key={page.id} className={`transition group ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100`} style={{ boxShadow: '0 1px 4px 0 rgba(30, 41, 59, 0.04)' }}>
                  <td className="px-6 py-4 font-semibold text-gray-900 capitalize align-middle">{page.name}</td>
                  <td className="px-6 py-4 text-gray-700 align-middle">{page.description}</td>
                  <td className="px-6 py-4 text-center text-gray-800 align-middle font-mono">{page.number_of_pages}</td>
                    ...
                    <div className="flex items-center gap-2 justify-end">
                      <button
                        ...
                        onClick={e => {
                          e.preventDefault();
                          setEditTarget(page);
                          setEditForm({ name: page.name, description: page.description || '', numberOfPages: page.number_of_pages });
                          setEditOpen(true);
                        }}
                      >
      ...
      {/* Edit Order Page Dialog */}
      {editOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-auto">
            <h3 className="text-lg font-bold mb-4 text-blue-900">Edit Order Page</h3>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                ...
                try {
                  const jwt = localStorage.getItem("jwt_token");
                  const res = await fetch(`${API_BASE_URL}/api/admin/order-pages/${editTarget.id}`, {
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: jwt ? `Bearer ${jwt}` : "",
                    },
                    body: JSON.stringify({ name: editForm.name, description: editForm.description, number_of_pages: parseInt(editForm.numberOfPages, 10) }),
                  });
                  if (!res.ok) throw new Error("Failed to update order page");
                  showToast({ message: "Order page updated successfully", type: "success" });
                  setEditOpen(false);
                  setEditTarget(null);
                  fetchOrderPages();
                } catch (err) {
                  showToast({ message: err.message || "Failed to update order page", type: "error" });
                } finally {
                  setEditing(false);
                }
              }}
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <label className="font-semibold text-blue-900 text-sm sm:text-base flex flex-col gap-1">
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(f => ({ ...f, numberOfPages: e.target.value }))}
                    disabled={editing}
                  />
                </label>
              </div>
              <label className="font-semibold text-blue-900 text-sm sm:text-base flex flex-col gap-1">
                ...
                <textarea
                  ...
                  placeholder="e.g. Single page document"
              ...
              <div className="flex flex-row justify-end gap-2 mt-2">
                ...
                <tr>
                  <td colSpan={3} className="text-center text-xs xs:text-sm sm:text-base px-4 py-1">No order pages found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {/* Create Order Page Dialog */}
      {dialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-auto">
            <h3 className="text-lg font-bold mb-4 text-blue-900">Create Order Page</h3>
            <form onSubmit={handleCreateOrderPage} className="flex flex-col gap-6 p-4 w-full max-w-lg mx-auto bg-white rounded-2xl shadow-xl border border-blue-100">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <label className="font-semibold text-blue-900 text-sm sm:text-base flex flex-col gap-1">
                  Name
                  <input
