...
const OrderLevels = () => {
  ...
  return (
    <React.Fragment>
      ...
      {loading ? (
        <div className="text-center py-8 text-blue-700">Loading order levels...</div>
      ) : error ? (
          ...
          <table className="w-full border-separate border-spacing-y-0 rounded-2xl overflow-hidden shadow-xl bg-white">
            ...
            <tbody className="divide-y divide-blue-100">
              {orderLevels.length > 0 ? orderLevels.map((level, idx) => (
                <tr key={level.id} className={`transition group ${idx % 2 === 0 ? 'bg-white' : 'bg-blue-50'} hover:bg-blue-100`} style={{ boxShadow: '0 1px 4px 0 rgba(30, 64, 175, 0.04)' }}>
                  <td className="px-6 py-4 font-semibold text-blue-900 capitalize align-middle">{level.name}</td>
