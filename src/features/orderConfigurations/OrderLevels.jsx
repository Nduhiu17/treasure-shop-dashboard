...
const OrderLevels = () => {
  ...
  const [editForm, setEditForm] = useState({ name: '', description: '', multiplier: '' });
  ...
  const fetchOrderLevels = () => {
    setLoading(true);
    setError("");
    fetch(`${API_BASE_URL}/api/order-levels`)
      .then((res) => res.json())
      .then((data) => setOrderLevels(data))
      .catch((err) => setError("Failed to fetch order levels"))
  ...
  const handleCreateOrderLevel = async (e) => {
    e.preventDefault();
    setCreating(true);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/admin/order-levels`, {
        method: "POST",
        headers: {
      ...
      showToast({ message: err.message || "Failed to create order level", type: "error" });
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <React.Fragment>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-2">
        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-900">Order Levels Management</h2>
        <Button
      ...
      {loading ? (
        <div className="text-center py-8 text-gray-500">Loading order levels...</div>
      ) : error ? (
        ...
        <div className="overflow-x-auto w-full">
          <table className="w-full border-separate border-spacing-y-0 rounded-2xl overflow-hidden shadow-xl bg-white">
            ...
            <tbody className="divide-y divide-gray-100">
              {orderLevels.length > 0 ? orderLevels.map((level, idx) => (
                <tr key={level.id} className={`transition group ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-gray-100`} style={{ boxShadow: '0 1px 4px 0 rgba(30, 41, 59, 0.04)' }}>
                  <td className="px-6 py-4 font-semibold text-gray-900 capitalize align-middle">{level.name}</td>
                  <td className="px-6 py-4 text-gray-700 align-middle">{level.description}</td>
                  <td className="px-6 py-4 text-center text-gray-800 align-middle font-mono">{level.level_price_multiplier}</td>
                    ...
                    <div className="flex items-center gap-2 justify-end">
                      <button
                        ...
                        onClick={e => {
                          e.preventDefault();
                          setEditTarget(level);
                          setEditForm({ name: level.name, description: level.description || '', multiplier: level.level_price_multiplier });
                          setEditOpen(true);
                        }}
                      >
      ...
      {/* Edit Order Level Dialog */}
      {editOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-md mx-auto">
            <h3 className="text-lg font-bold mb-4 text-blue-900">Edit Order Level</h3>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                ...
                try {
                  const jwt = localStorage.getItem("jwt_token");
                  const res = await fetch(`${API_BASE_URL}/api/admin/order-levels/${editTarget.id}`, {
                    method: "PUT",
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: jwt ? `Bearer ${jwt}` : "",
                    },
                    body: JSON.stringify({ name: editForm.name, description: editForm.description, level_price_multiplier: parseFloat(editForm.multiplier) }),
                  });
                  if (!res.ok) throw new Error("Failed to update order level");
                  showToast({ message: "Order level updated successfully", type: "success" });
                  setEditOpen(false);
                  setEditTarget(null);
                  fetchOrderLevels();
                } catch (err) {
                  showToast({ message: err.message || "Failed to update order level", type: "error" });
                } finally {
                  setEditing(false);
                }
              }}
